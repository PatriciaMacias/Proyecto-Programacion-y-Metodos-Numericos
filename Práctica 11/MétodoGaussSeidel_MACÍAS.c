...
float ERRORAbsoluto1,ERRORAbsoluto2,ERRORAbsoluto3;
int i,j;
int main()
{
...
for(j=0;j<5;j++){
    if(j==0){ 
        ...
        ERRORAbsoluto1=fabs((x_iteracion1-xinicial1)/(x_iteracion1))*100;
        xinicial2 = x_iteracion2;
        x_iteracion2 = ((-A[1][0]*x_iteracion1)-(A[1][2]*x_iteracion3)+b[1])/A[1][1];
        ERRORAbsoluto2=fabs((x_iteracion2-xinicial2)/(x_iteracion2))*100;
        xinicial3 = x_iteracion3;
        x_iteracion3 = ((-A[2][0]*x_iteracion1)-(A[2][1]*x_iteracion2)+b[2])/A[2][2];
        ERRORAbsoluto3=fabs((x_iteracion3-xinicial3)/(x_iteracion3))*100;
        printf("x1 en la iteracion %d es = a %f, su error absoluto es= %f %%\n",j+1,x_iteracion1,ERRORAbsoluto1);
        printf("x2 en la iteracion %d es = a %f, su error absoluto es= %f %%\n",j+1,x_iteracion2,ERRORAbsoluto2);
        printf("x3 en la iteracion %d es = a %f, su error absoluto es= %f %%\n",j+1,x_iteracion3,ERRORAbsoluto3);
        }
    }
}